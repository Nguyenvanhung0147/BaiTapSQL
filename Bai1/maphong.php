...
<tr>
	<td align="center">Mã Phòng: <input type="text" name="maphong" size="30" 
...
<table class="table table-bordered table-striped table-responsive-stack" style="text-align: center;margin: auto;">
   <thead class="thead-dark">
	  <tr>
		 <th style="background-color: black;color :white">Mã Loại Phòng</th>
		 <th style="background-color: black;color :white">Tên loại Phòng </th>
   ...
   <tbody>
		  	...
		  	if(empty($_GET['maphong']))
		 {
		  ...
		  while ($row = mysqli_fetch_array($result))
		  {
			  ...
			  {	echo "<tr>";
				  ...
				  echo "<td>$row[1]</td>";
				  echo "<td>
					 <a href='deletephong.php?id=$row[MAPHONG]'><i class='fas fa-trash-alt'></i></a>
					 <a href='editphong.php?id=$row[MAPHONG]'><i class='fas fa-edit'></i></a>
					 </td>";
				  echo "</tr>";
			  }
			  else
		...
		$query="Select * 
		  ...
		  $timkiem=mysqli_query($dbc,$query);		
			...
			{	$rows=mysqli_num_rows($timkiem);
				echo "<div align='center'><b>Có $rows Loại phòng được tìm thấy.</b></div>";
	...
	else echo "<div><b>Không tìm thấy loại phòng này.</b></div>";
}
